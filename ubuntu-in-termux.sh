...
install1 () {
	...
	UBUNTU_VERSION_NAME="$(echo ${UBUNTU_VERSION} | awk -F __ '{printf "%s\n",$2}')"
	UBUNTU_VERSION_NUM="$(echo ${UBUNTU_VERSION} | awk -F __ '{printf "%s\n",$1}')"
	if [ -d "$directory" ];then
	first=1
		...
		printf "\x1b[38;5;214m[${time1}]\e[0m \x1b[38;5;203m[ERROR]:\e[0m \x1b[38;5;87m Please install aria2.\n"
		printf "\e[0m"
		exit 1
	fi
	if [ "$first" != 1 ];then
